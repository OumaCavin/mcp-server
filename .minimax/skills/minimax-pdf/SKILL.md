...
name: minimax-pdf
description: >
  Unified PDF skill — generate, reformat, fill, and read existing PDFs. GENERATE / CREATE: "make a
  PDF", "generate a report", "write a proposal", "create a resume", "beautiful PDF", "professional
  document", "cover page", "data report", "可视化报告", "图文报告", "评测报告". LaTeX academic
  thesis: "毕业论文 LaTeX 模板", "学校论文格式", "封面 PDF 合并", "参考文献 GB/T 7714",
  "页眉页脚奇偶页", "三线表", "tectonic 编译". REFORMAT: "reformat this document", "apply our
  style", "convert this Markdown/text to PDF", "make this doc look good", "re-style this PDF",
  "重排版", "markdown 转 PDF". FILL: "fill in the form", "fill out this PDF", "complete the form
  fields", "write values into PDF", "what fields does this PDF have". READ: "read PDF", "extract
  text", "parse PDF", "summarize PDF", "get tables", "search PDF", "scanned PDF", "what's in this
  PDF", "读 PDF", "提取", "解析", "抽取", "抠表格", "看一下这个 PDF", "总结 PDF". Generation routes
  render via Playwright headless Chromium (HTML+CSS → PDF), tectonic/xelatex (LaTeX), or
  pypdf/reportlab (forms). Read route uses pdfplumber by default, escalating to the agent
  runtime's image-understanding tool for scanned / chart-heavy PDFs.
metadata:
  version: '3.0'
...
> **3. Chart pages and complex financial tables — image understanding, ONE PAGE PER CALL, mandatory.** Any page
...
| **Image-understanding escalation** — scanned / image-only PDFs, chart values, broken reading order | [`docs/vision-guide.md`](docs/vision-guide.md)   | `python3 -m scripts.read_pdf_vision --input report.pdf --pages 1-30`, then call the agent image-understanding tool on emitted chunks |
...
> Default to **pdfplumber**. Only escalate to image understanding when the text path is insufficient
> (`(cid:NNN)` glyphs, empty strings, charts that matter, magazine-style layout). The helper script
> only prepares page images; the agent runtime's built-in image tool performs interpretation.
...
`read_pdf_vision.py` additionally requires `pdf2image`, Pillow, and poppler. It does **not** call
a separate CLI, daemon, or hand-written tool endpoint. Details: [`docs/vision-guide.md`](docs/vision-guide.md).
